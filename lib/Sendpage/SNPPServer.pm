...
sub HandleSNPP
{
    ...
    $hostname = $sock->sockhost if $hostname eq "";
	...
	while (1) {
		...
		my $queued = write_queued_pages $sock
		    $pipe, $caller, $text, $config, $log, $DEBUG, @PINS;
...
sub write_queued_pages
{
	...
	if (length($text) > $pagingcentral->maxchars()) {
	    $log->do('debug',
		     "Splitting due to PC '%s' maxchar " . "limit: %d",
		     $pc, $pagingcentral->maxchars())
		if $DEBUG;
	    ...
	    if ($splittext ne "") {
		$log->do('warning',
			 "threw away %d extra chars at the end of a page with more than %d splits",
			 length($splittext), $chunks);
	    }
	} else {
	    push @pages, $text;
	}
	...
	foreach $pagetext (@pages) {
		...
		$log->do('info',
			 "$pc/$file: state=Queued, to=%s, from=%s(%s), size=%d",
			 join("," => @tolist), $repfrom, $client, length($pagetext));
	    }
	}
	print $pipe "$pc\n" if defined $pipe;
    }
